...
export function buildHelpTopic(topicRaw: string): string {
  ...
  if (['daily', 'day', '每日', '今日', 'fun', '娱乐', '好玩', '小考', '训练'].includes(topic)) {
    return [
      ...
      '/csutility /cstactic /csclutch /cseco /cscall /csreview /csloadout - 道具/战术/残局/经济局/指挥口令/复盘切片/套餐',
      '/csknife 或 今日发刀/发刀/.d - 每日刀具，覆盖全部刀型和常见刀皮，优先发专属美图池',
      '/mokoko 或 每日木柜子 - MyGO!!!!! / Ave Mujica 每日角色，优先发专属卡面美图',
      '/genshin 或 每日原神角色 - 每日原神角色签，优先发专属角色美图',
      '/cold /book /poem - 每日冷知识 / 书摘 / 古诗词',
      '/movie /music /history - 每日影视台词 / 音乐知识 / 历史上的今天',
      '/science /duel - 每日科学知识 / 决战紫禁之巅（你和机器随机武器对决）',
  ...
  if (['memory', 'mem', 'cache', 'trace', '内存', '缓存', '记忆', '诊断'].includes(topic)) {
    return [
      ...
      '/mem cache status - 回复缓存池容量、TTL、命中率、策略Top',
  ...
  handler: (ctx) => {
    ...
    if (ctx.command === 'help' || fuzzy === 'help') {
      ...
      const helpText = [
        ...
        '',
        '对话:',
        '  /talk <内容> - 直接对话（兼容 /ai /ask /chat）',
        '  私聊我 - 直接进入独立私聊上下文',
        '  /voice <内容> - 语音回复',
        '  /tts <内容> - 同上',
        '  /draw <提示词> - NAI画图（兼容 /画图 /nai，可加 --w --h --steps --scale --seed --neg）',
        '  /search <关键词> - 联网搜索',
        '  @我 <内容> - @触发',
        '  /reset - 清除记忆',
        '  /presets - 预设列表',
        ...
        '  /csutility /cstactic /csclutch /cseco /cscall /csreview - 道具/战术/残局/经济局/指挥口令/复盘切片',
        ...
        '  /movie - 每日影视台词；/music - 每日音乐知识；/history - 历史上的今天',
        '  /science - 每日科学知识；/duel - 每日决战紫禁之巅',
        ...
        '  也可以直接说: 今日CS / 今天抽个选手 / 今日地图 / 今天用什么枪 / 今日皮肤 / 今日经济局 / 今日指挥口令 / 今日复盘 / 今日发刀 / 每日木柜子 / 每日原神角色 / 每日冷知识 / 每日书摘 / 每日古诗词 / 每日影视台词 / 每日音乐知识 / 历史上的今天 / 每日科学知识 / 决战紫禁之巅 / 今天怎么练枪 / 来个CS小考',
        '',
        '趣味:',
        '  /roll [N|NdM] - 骰子',
        '  /luck - 运势',
        ...
        '  /mem - 内存/上下文/RAG/回复缓存/用户画像缓存状态，/mem health 看健康，/mem plan 出只读维护计划，/mem cache status 看缓存池，/mem cache <消息> 预检回复缓存，/mem check <消息> 预检RAG召回',
        '  /mem recent [条数] - 看最近上下文和RAG索引',
        '  /profile / set team|player|map|tone|note / clear - 当前会话用户自填长期偏好画像，会用于个性化但不当实时事实证据',
        '  /time - 时间',
        '  /stats - 群统计',
        '  /me - 我在群的活跃度',
        ...
        '知识库:',
        '  /kb search <关键词> - 检索',
        '  /kb route <消息> - 预检会带入哪些风格/话题知识、命中诊断和时效风险',
